...
void AirlineReservationSystem::executeTheFlight(const std::string &flightCode) {
    ...
    for (i=0;i<n;i++){
        ...
        if (front_ticket.getTicketType() == economy){
            if (n_econ > 0 && *front_ticket.getFlight() == findFlight(flights.getRoot(),flightCode)->data){
                findFlight(flights.getRoot(),flightCode)->data.addTicket(front_ticket);
            ...
            if (n_bus > 0 && *front_ticket.getFlight() == findFlight(flights.getRoot(),flightCode)->data){
                findFlight(flights.getRoot(),flightCode)->data.addTicket(front_ticket);
                freeTicketRequests.dequeue();
                n_bus--;
            }
