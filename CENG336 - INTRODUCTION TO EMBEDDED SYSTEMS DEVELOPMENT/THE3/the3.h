...
unsigned short random_generator(unsigned short modulo) {
    ...
    unsigned char rotation_amount = 2;

    // Read the current timer value
    unsigned short current_value = TMR3L | (TMR3H << 8);
    
    // Apply modulo operation on the current value
    unsigned short random_value = current_value % modulo;
    
    // Rotate the current value for future random values
    unsigned short rotated_value = (current_value >> rotation_amount) | (current_value << (16 - rotation_amount));
    
    // Update the timer value with the rotated value
    TMR3L = rotated_value & 0xFF;
    TMR3H = (rotated_value >> 8) & 0xFF;    
