...
{
    //TerminateTask();
    //LCD TASK: WHENEVER STATUS CHANGED, SIGNALED BY LCD_EVENT TO REFRESH LCD
    ADCON1 = 0x8E;
    ADCON0 = 0x00;
    InitLCD();   
    ...
    while(1)
    {
        //SIGNALED AT EVERY STATUS CHANGE, OR BY LCD FUNCTIONS WHEN THEY ARE DONE
        WaitEvent(LCD_EVENT);
...
void Delay_LCD_ms(unsigned int delay)
{
  ...
  WaitEvent(ALARM_EVENT_LCD);
  GetEvent(LCD_ID, &LCDevent);
  if (LCDevent & ALARM_EVENT_LCD)
    ClearEvent(ALARM_EVENT_LCD);
  return;
}
...
{
    //TXREG = 'A';
    SetRelAlarm(ALARM_TSK3, 100, 350);
