...
std::string operator+(const HighSpeedTrain &h1, const HighSpeedTrain &h2){
    if (h1.to != h2.from) return "Transfer is not possible!";
    else {
        std::string arrow = " -> ";
        std::string in = " in ";
        std::string mins = " mins.";
        return h1.from + arrow + h1.to + arrow + h2.to + in + std::to_string(h1.eta + h2.eta) + mins; 
    }
}
...
bool operator<(const HighSpeedTrain &h1, const HighSpeedTrain &h2){
    if (h1.from != h2.from) return (h1.from < h2.from);
    else {
        if (h1.to != h2.to) return (h1.to < h2.to);
        else return (h1.eta < h2.eta);
    }
}
