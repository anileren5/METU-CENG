...
import {updateItem} from "@/utils/api/services";
...
const categories = ['vehicle', 'computer', 'phone', 'private lesson'];
...
const UpdateItemPopup = ({ item, handleSave, isOpen}) => {
    const classes = useStyles();
    const [updatedItem, setUpdatedItem] = useState(item);
    const old_price = item.priceInTermsOfTL;
    const oldPriceInOriginalCurrency = item.price;
    const oldCurrency = item.currency;
    const [shareContact, setShareContact] = useState(item.shareContact);
    ...
    const handleUpdate = async () => {
        ...
        temp.priceInTermsOfTL = temp.currency === "TL" ? 1 * temp.price : (temp.currency === "USA" ? 32 * temp.price : 35 * temp.price);
        temp.shareContact = shareContact;

        await updateItem(temp._id, {
            updatedItem: temp,
            old_price: old_price,
            oldCurrency: oldCurrency,
            oldPriceInOriginalCurrency: oldPriceInOriginalCurrency
        });
        handleSave(temp);
    };
    ...
    const handleListNameChange = (e, index) => {
        ...
        setUpdatedItem(prevItem => ({
            ...prevItem,
            listFields: [
                ...prevItem.listFields.slice(0, index),
                { ...prevItem.listFields[index], name: value },
                ...prevItem.listFields.slice(index + 1)
            ]
        }));
    };

    const handleListItemChange = (e, listIndex, itemIndex) => {
        const { value } = e.target;
        setUpdatedItem(prevItem => ({
            ...prevItem,
            listFields: [
                ...prevItem.listFields.slice(0, listIndex),
                {
                    ...prevItem.listFields[listIndex],
                    list: [
                        ...prevItem.listFields[listIndex].list.slice(0, itemIndex),
                        { ...prevItem.listFields[listIndex].list[itemIndex], value: value },
                        ...prevItem.listFields[listIndex].list.slice(itemIndex + 1)
                    ]
                },
                ...prevItem.listFields.slice(listIndex + 1)
            ]
        }));
    };

    const handleFlatFieldChange = (e, index, fieldName) => {
        const { value } = e.target;
        const updatedFlatFields = updatedItem.flatFields.map((field, i) => {
            if (i === index) {
        ...
        setUpdatedItem(prevItem => ({
            ...prevItem,
            flatFields: updatedFlatFields
        }));
    };

    const handleNestedFieldNameChange = (e, index) => {
        const { value } = e.target;
        setUpdatedItem(prevItem => ({
            ...prevItem,
            nestedFields: prevItem.nestedFields.map((nestedField, i) => {
                if (i === index) {
    ...
    const handleNestedFieldChange = (e, nestedIndex, fieldIndex, fieldKey) => {
        const { value } = e.target;
        setUpdatedItem(prevItem => ({
            ...prevItem,
            nestedFields: prevItem.nestedFields.map((nestedField, index) => {
                if (index === nestedIndex) {
                    return {
                        ...nestedField,
                        fields: nestedField.fields.map((field, i) => {
                            if (i === fieldIndex) {
    ...
    return (
        <Dialog open={isOpen} onClose={() => handleSave(updatedItem)} aria-labelledby="update-item-dialog-title">
            <DialogTitle id="update-item-dialog-title">Update Item</DialogTitle>
            <DialogContent className={classes.dialogContent}>
                ...
                <div className={classes.imageContainer}>
                    <img className={classes.image} src={updatedItem.image || defaultImageBase64} alt="Item"/>
                    <input
                        accept="image/*"
                ...
                {updatedItem.nestedFields && updatedItem.nestedFields.map((nestedField, index) => (
                    <Paper key={index} className={classes.subform}>
                        <TextField
                            label={`Nested Property Name`}
                            value={nestedField.name}
                            onChange={(e) => handleNestedFieldNameChange(e, index)}
                            className={classes.textField}
                        />
                        {nestedField.fields.map((field, fieldIndex) => (
                            <Grid key={fieldIndex} container spacing={2}>
                                <Grid item xs={6}>
                                    <TextField
                                        label={`Property Name`}
                                        value={field.fieldName}
                                        onChange={(e) => handleNestedFieldChange(e, index, fieldIndex, 'fieldName')}
                                        className={classes.textField}
                                    />
                                </Grid>
                                <Grid item xs={6}>
                                    <TextField
                                        ...
                                        onChange={(e) => handleNestedFieldChange(e, index, fieldIndex, 'fieldValue')}
                                        className={classes.textField}
                                    />
                                </Grid>
                            </Grid>
                ...
                {updatedItem.flatFields && updatedItem.flatFields.map((field, index) => (
                    <Paper key={index} className={classes.subform}>
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <TextField
                                    label={`Property Name`}
                                    value={field.fieldName}
                                    onChange={(e) => handleFlatFieldChange(e, index, 'fieldName')} // Pass 'fieldName'
                                    className={classes.textField}
                                />
                            </Grid>
                            <Grid item xs={6}>
                                <TextField
                                    label={`Value`}
                                    value={field.fieldValue}
                                    onChange={(e) => handleFlatFieldChange(e, index, 'fieldValue')} // Pass 'fieldValue'
                                    className={classes.textField}
                                />
                            </Grid>
                        </Grid>
                ...
                {updatedItem.listFields && updatedItem.listFields.map((listField, index) => (
                    <Paper key={index} className={classes.subform}>
                        <TextField
                            label="Property Name"
                            ...
                            className={classes.textField}
                        />
                        {listField.list.map((listItem, valueIndex) => (
                            <TextField
                                key={valueIndex}
                                label={`Value ${valueIndex + 1}`}
                                value={listItem.value}
                                onChange={(e) => handleListItemChange(e, index, valueIndex)}
                                className={classes.textField}
                            />
                        ))}
                    </Paper>
            ...
            <DialogActions>
                ...
                <Button onClick={() => handleSave(updatedItem)} color="default">
                    Cancel
                </Button>
            </DialogActions>
        </Dialog>
