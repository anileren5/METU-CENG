import { getAllItems } from "@/utils/api/services";
...
export default function UnauthenticatedUserDashboard({selectedCategory}) {
    ...
    const fetchInitialData = async () => {
        try {
            ...
            let maxInTermsOfTL = 0;
            data.forEach(item => {
                if (item.priceInTermsOfTL > maxInTermsOfTL) {
                    maxInTermsOfTL = item.priceInTermsOfTL;
                }
            });

            if (selectedCurrency === "TL") {
                setMaxPrice(maxInTermsOfTL);
                setPriceRange([0, maxInTermsOfTL]);
            }
            else if (selectedCurrency === "USD") {
                setMaxPrice(maxInTermsOfTL / 32);
                setPriceRange([0, maxInTermsOfTL / 32]);
            }
            else if (selectedCurrency === "EUR") {
                setMaxPrice(maxInTermsOfTL / 35);
                setPriceRange([0, maxInTermsOfTL / 35]);
            }
    ...
    const handleCheckboxChange = (event) => {
        // Implement checkbox change handling
        // You might have state to track which checkboxes are checked
    };
    ...
    const applyFilters = () => {
        if (selectedCategory === "vehicle") {
            ...
            if (selectedCategory === "vehicle") {
                if (fuelType !== "All") {
                    const fuelTypeFilter= (item) => {
                        let flag = false;
                        for (let i = 0; i < item.flatFields.length; i++) {
                            if (item.flatFields.at(i).fieldName === "Fuel Type") {
                                if (item.flatFields.at(i).fieldValue === fuelType){
                    ...
                    for (let i=0; i<items.length; i++) {
                        if (fuelTypeFilter(items.at(i))) {
                            remainings.push(items.at(i));
                        }
                    }
                }
            }
            ...
            let temp = []
            // Price filtering
            remainings.forEach(item => {
                if (selectedCurrency === "TL") {
                    if (item.priceInTermsOfTL >= priceRange[0] && item.priceInTermsOfTL <= priceRange[1]) {
                        temp.push(item);
                    }
                }
                else if (selectedCurrency === "USD") {
                    if ((item.priceInTermsOfTL / 32)  >= priceRange[0] && (item.priceInTermsOfTL / 32)  <= priceRange[1]) {
                        temp.push(item);
                    }
                }
                else if (selectedCurrency === "EUR") {
                    if ((item.priceInTermsOfTL / 35)  >= priceRange[0] && (item.priceInTermsOfTL / 35)  <= priceRange[1]) {
                        temp.push(item);
                    }
                }
            });
    ...
    async function resetFilters() {
        ...
        else if (selectedCategory === "phone") {
        }
        else if (selectedCategory === "private lesson") {
        }
    }

    return (
        <div style={{ display: 'flex', height: '100vh' }}>
            <div style={{flex: 2, marginRight: "2vw", marginTop: "6vh", height: "100vh"}} className={classes.root}>
                ...
                <div>
                    <h3>Currency for Price Range</h3>
                    <Select
                ...
                <div>
                    <FormGroup>
                        {selectedCategory === "vehicle" &&
                            <>
                                <Paper elevation={3} style={{ padding: '20px', marginBottom: '20px', marginTop: "10px", borderColor: "black", backgroundColor:"white"}}>
            ...
            <div style={{flex: 9}}>
                ...
                {popupOpen &&
                    <ItemPopup isContactDetailsSecret={true} open={popupOpen} item={selectedItem} onClose={() => {
                        setSelectItem(null);
                        setPopupOpen(false);
                    }}></ItemPopup>}
                {items && items.map((item) => (
                    ...
                    <Card key={item._id} style={{marginBottom: '20px', display: 'flex'}}>
                        ...
                        <CardContent style={{flex: '1', paddingLeft: '20px'}}>
                            ...
                            <Button style={{backgroundColor: "#25aae1", color: "white"}} onClick={() => {
                                setSelectItem(item);
                                setPopupOpen(true)
                            }}>View Details</Button>
                        </CardContent>
